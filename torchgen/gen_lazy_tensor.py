...
) -> Tuple[List[OperatorName], Dict[str, Any]]:
...
def validate_shape_inference_header(
    ...
    missing_decls = [
        ...
        if decl not in shape_infr_decl_lines
    ]
    if missing_decls:
...
def run_gen_lazy_tensor(
    ...
    full_codegen, non_native = parse_native_functions_keys(source_yaml, grouped_native_functions)
    ...
    lazy_ir_generator = lazy_ir_generator(backend_indices[backend_key], backend_name, node_base)
    ...
    fm.write_with_template(
        ...
        lambda: {
            ...
            "ir_declarations": list(concat_map_codegen(lazy_ir_generator, grouped_native_functions)),
            "namespace_prologue": ns_helper.prologue,
            "namespace_epilogue": ns_helper.epilogue,
        },
    )
    ...
    fm.write_with_template('LazyNonNativeIr.h', 'LazyNonNativeIr.h', lambda: {
        'lazy_non_native_ir_inc': [f'#include <{path}>' for path in [
                "torch/csrc/lazy/core/ir.h",
        ...
        'non_native_ir_nodes': dest.generate_non_native_lazy_ir_nodes(non_native, lazy_ir_generator),
        'namespace_prologue': ns_helper.prologue,
        'namespace_epilogue': ns_helper.epilogue,
    })
